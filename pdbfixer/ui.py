        ...
        if parameters.getfirst('type') == 'local':
            ...
            try:
                ...
                handler.sendResponse(
                    ...
                    "<p>{}</p>".format(e) + loadHtmlFile("error.html"))
...
def deleteChainsPageCallback(parameters, handler):
    ...
    deleteIndices = [
        i for i in range(numChains) if 'include' + str(i) not in parameters
    ]
...
def convertResiduesPageCallback(parameters, handler):
    for i in range(len(fixer.nonstandardResidues)):
        if 'convert' + str(i) in parameters:
            fixer.nonstandardResidues[i] = (fixer.nonstandardResidues[i][0],
                                            parameters.getfirst('residue' +
                                                                str(i)))
...
def saveFilePageCallback(parameters, handler):
    if 'pdb' in parameters:
        ...
        try:
            app.PDBxFile.writeFile(fixer.topology, fixer.positions, output,
                                   True)
        except AssertionError:
            print
...
def displayDeleteChainsPage():
    ...
    uiserver.setContent(header + loadHtmlFile("removeChains.html") %
                        (numChains, table))
...
def displayConvertResiduesPage():
    ...
    for i in range(len(fixer.nonstandardResidues)):
        ...
        for res in replacements:
            ...
            if res == replaceWith:
                selected = ' selected'
            options += '<option value="%s"%s>%s</option>' % (res, selected,
                                                             res)
...
def displayAddHydrogensPage():
    ...
    if fixer.topology.getUnitCellDimensions() is not None:
        dimensions = "<tr><td>Crystallographic unit cell:</td><td>%.3f</td><td>%.3f</td><td>%.3f</td></tr>" % fixer.topology.getUnitCellDimensions(
    ...
    sizeRange = tuple(
        ...
        for i in range(3))
    dimensions += "<tr id='boxContainingAllAtoms'><td>Box containing all atoms:</td><td>%.3f</td><td>%.3f</td><td>%.3f</td></tr>" % tuple(
        x.value_in_unit(unit.nanometer) for x in sizeRange)
    uiserver.setContent(header +
                        loadHtmlFile("addHydrogens.html") % dimensions)
